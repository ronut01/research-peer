...
현재 phase: **v1.1 `rp` Remote Control launcher and peer workflow locally verified; peer acceptance pending**
...
- Claude Code 2.1.231–2.1.234와 Anthropic 공식 Channels/Remote Control/plugin/skills/MCP 검증
...
- no-argument `rp` session-scoped Remote Control launcher plus canonical Remote-Control-off `research-peer`, guided slash UX, rich help, explicit continue/resume
...
- PASS: **[SERVER-VERIFIED]** current user install provides `rp` as an owned executable launcher; interactive no-argument dispatch is `start --remote-control`, subcommands report the same 1.1.0 version, uninstall/residue handling includes it, and installer tests refuse both overwrite and PATH shadowing conflicts
