...
- 고정된 공식 GitHub repository 기준의 user-scope runtime/plugin/skill self-update
- 같은 물리 서버의 다른 Unix 사용자와 다른 물리 서버
- 선택적 Remote Control launcher; Remote Control과 P2P는 독립
- CLI inbox/history, room별 connection/session/delivery 상태
- 가장 최근 live session으로 broadcast를 결정적으로 전달하고 stale/alias 경쟁 binding 정리
- config와 실제 daemon listener 불일치 진단, wildcard/loopback advertise 보호, 기본 24시간 invite
...
research-peer room configure ROOM [--auto-answer on|off] [--disclosure LEVEL] [--note TEXT]
...
research-peer send
research-peer inbox [--room ROOM] [--all] [--consume] [--json]
research-peer history [--room ROOM] [--json]
research-peer logs
research-peer version
research-peer update [--check] [--yes]
...
- 여러 room과 session을 저장할 수 있지만 Claude session당 활성 room은 최대 하나다.
- 같은 room/alias를 재등록하면 이전 binding을 retire한다. 인자 없는 register는 `RESEARCH_PEER_SESSION_ID`를 우선 사용하며 없으면 live binding이 아닐 수 있음을 경고한다.
- broadcast inbound는 수신 시점의 가장 최근 live session 하나에 durable하게 배정한다. session이 없으면 inbox에 보류하고 이후 등록된 적합 session이 가져간다.
...
Invite는 URL-safe encoded, versioned JSON이며 protocol version, room UUID/display name, creator endpoint, transport, creator certificate/public-key fingerprint, expiry, one-time token을 포함할 수 있다. 기본 유효기간은 24시간이다. token은 log/help/Git에 출력하지 않으며 creator state에는 원문 대신 hash를 저장한다.
...
- payload 상한은 256 KiB, clock skew 허용은 ±5분, 보관 replay window는 최소 24시간이다.
...
- receiver는 inbox/dedup 저장 후 선택된 live session binding에 Channel event를 노출한다. CLI `inbox`는 Channel과 무관하게 pending body를 읽을 수 있고 `history`는 inbound/outbound/automatic reply audit을 제공한다.
...
- `full`: Claude가 답변 text를 작성할 수 있는 고위험 명시적 opt-in이다.
- credential, 환경변수, transcript, file content, invite/token/endpoint, `~/.ssh`, 명령 실행과 configuration 변경은 어느 수준에서도 자동응답 대상이 아니다.

Automatic generation은 `type=QUESTION`, `reply_required=true`, `owner_attention=false`인 inbound 하나에만 `ANSWER`를 생성할 수 있다. `ANSWER`는 terminal이며 자동 `QUESTION`을 만들 수 없다. incoming `automation_depth + 1`을 보존하고 최대 4를 넘으면 owner에게 escalation한다. `(room_id, request_id)`는 한 번만 auto-answer되며 question, answer, disclosure, timestamp를 audit한다. 정책 밖 질문은 모호한 답을 만들지 않고 owner attention으로 남긴다.
...
`research-peer update`는 source override를 받지 않고 `https://github.com/ronut01/research-peer`만 shallow temporary checkout한다. mutation 전에 Git origin, commit, project/plugin identity, Python/package/plugin/marketplace version 일치를 검사하고 downgrade를 거부한다. `--check`는 read-only이며, 기본 mutation은 local TTY confirmation을 요구한다. 명시적으로 호출된 `/research-peer:update`만 `--yes`를 사용할 수 있다. update는 identity/private key, room/peer/history/outbox/config/log와 project/artifact를 보존하고 기존 daemon이 실행 중일 때만 stop/install/restart한다. 현재 Claude process의 development Channel/skill hot reload는 주장하지 않고 session restart를 안내한다.
...
23. CLI inbox가 SQLite 직접 접근 없이 pending message body를 표시
24. 여러 live session에서 broadcast가 억제되지 않고 하나에 deterministic delivery
25. session register 환경변수 사용, stale/current 표시, competing alias retire
